...
cuánto se mezcla lo restaurado con el original: la voz tiene que seguir siendo
la de él, no "de locutor". 0 = no se toca.
...
RESTAURACION_NIVEL_DEFAULT = 0.7  # 19/09: al 100 % Naza oyó la voz "temblar"; 0,7 diluye lo que el modelo inventa
...
def restaurar(entrada: Path, salida: Path, nivel: float | None = None, registro: Path | None = None, log=None) -> dict:
    ...
    entorno.setdefault("HF_HOME", r"D:\vitacora-modelos")
    proceso = subprocess.run(comando, capture_output=True, text=True, env=entorno, cwd=str(RAIZ))
