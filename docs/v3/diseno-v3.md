...
| Avance a la vista | Por etapas, **sin números** (Naza, 26/09: cualquier número se lee como promesa): "Terminamos tu infancia. Ya está guardada en tu libro." Progreso dotado desde el primer mensaje: portada, dedicatoria y línea de tiempo salen de la ficha. Los conteos de lo ya hecho ("contaste 9 historias") se mantienen. |
...
**Dashboard (antes de escribir):** 1) Tu libro por dentro (índice con título, subtítulo y 2-3 líneas de las historias de cada capítulo, **sin números**); 2) Cosas para confirmar (máx. 15: nombres con 3 s de audio, fechas, temas delicados agrupados, dudas de comprensión, y **"¿esto fue antes o después de irte?"** cuando una respuesta tiene señales de los dos lados en el modo migrante —aprobado por Naza 26/09—); 3) Tus actividades (oficio/pasión); 4) Tus respuestas, opcional (el texto tal cual, para corregir). Botón "Dejá todo como está"; si no entra en 5 días, se escribe con lo que hay. Duda "importante" por regla: nombre de persona con historia, año de la línea de tiempo, sensible, o contradicción ficha/audio.
