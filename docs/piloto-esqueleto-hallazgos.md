...
- **D1. Lo que la familia corrige o excluye en el dashboard tiene que llegar al libro.** Hoy la fábrica ignora `edicion.correcciones` y `edicion.excluidas` (decisión del 13/09). Tarea prioritaria después del libro de Naza: la fábrica los aplica; formato de `correcciones` a acordar con Joaquín. **Hecho (ajuste G, 25/09):** `excluidas` quedan afuera del libro y del audiolibro como una reservada (una principal se lleva sus repreguntas; un capítulo vacío se cae), en producción (`generarPaquete`, `imprenta`) y en el v2 (`prueba-reparto`/`releer-libro`); `correcciones` va tal cual (texto libre, sin formato a acordar) al escritor, al editor y al lector final. De paso: el audiolibro de producción ya no hace sonar las respuestas reservadas (antes elegía por nombre de archivo y no las miraba). **Revisión:** los borradores cacheados llevan un manifiesto (`borradores.json`: capítulos por posición + huella de respuestas, excluidas y correcciones) y no se reusan si no coincide; una respuesta sin nada publicable (reservada entera) cuenta como ida igual que una excluida; `correcciones` topeado a 4000 caracteres.
  - Pendiente: `ampliarExcluidas` con dos respuestas principales en la misma orden: excluir una se lleva las repreguntas de las dos.
  - Pendiente: las fotos de un capítulo que se cae por excluidas desaparecen del libro sin aviso a la familia.
  - Pendiente: `audiosPublicables` es fail-open (un audio que no es de ninguna respuesta conocida suena); más seguro armar la lista desde los `audio_path` publicables.
  - Pendiente (fuera del ajuste): `limpiarBorradores` borra `borrador_libro.md` al entregar, y `imprenta.ts:54` lo necesita para el impreso después de la entrega.
  - Pendiente (fuera del ajuste): `imprenta.ts` busca las fotos por el nombre del capítulo en el guion y no por el título final: un capítulo renombrado pierde sus fotos.
  - Pendiente (fuera del ajuste): la muestra de audio de la landing no mira `excluidas`.
