...
|---|---|---|---|
| 1 | Modelos por paso (`PasoV2`) y precio de Sonnet 5 en un solo lugar | `672ee05` | Ninguno de fondo. `global-constraints.md` menciona un paso "evaluación reducida" con Haiku que no existe en `PasoV2` de esta tarea — no se inventó, puede ser de otra tarea |
...
- **Historia grande repetida en una sola línea.** En la lista de "ya hechas", dos eventos históricos de la misma etapa (por ejemplo tu pandemia y el Mundial) quedan con la misma cabeza y se ven como una sola línea. No se pierde nada real (las dos preguntas ya se hicieron, el punto de la lista es "no vuelvas"), pero conviene un arreglo chico (~40 caracteres) antes del piloto para que se distingan.
