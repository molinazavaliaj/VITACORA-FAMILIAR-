...
| **12/09** | **Opus 4.6** | **~USD 22** | **NO es el producto**: nada en el repo usó nunca Opus 4.6. **Confirmado en la consola el 18/09: USD 21,89 con la key `tricky-noise-api-key`** — 0,02 input + 9,52 escritura de caché + 10,97 lectura de caché + 1,37 output; 23,5 M tokens de entrada y 55 K de salida. Ese perfil es una sesión de Claude Code releyendo contexto todo el día, no un libro. La key estaba como **variable de entorno global de Windows** (`ANTHROPIC_API_KEY`, usuario) en la PC de Naza → el Claude Code abierto desde terminal cobraba a la API en vez de a la suscripción. **Variable borrada el 18/09; la key `tricky-noise-api-key` se revoca** (la fábrica local ya usa `fabrica-local`). Casi la mitad del gasto del mes. |
