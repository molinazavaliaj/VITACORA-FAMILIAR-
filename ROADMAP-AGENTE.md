...
- [x] **T3.16** — web: re-aplicar la validacion de precios de T3.6 sobre la estructura actual de main (y corregir el default de AR)
  - deps: —
  - tamaño: S
  - hecho-cuando: `cd web && npx vitest run && npx tsc --noEmit` desde la raiz del repo: exit 0, con tests que cubran un valor INVALIDO para cada precio (comillas, coma de miles, cero, negativo, Infinity) incluido el del PDF, y que verifiquen que el default de AR ya no esta por debajo del precio de la casa
  - nota: Rama web-precios-invalidos-2 desde la rama actual web-webhook-mp-log (1cda36a), commit dcea589, sin push: la rama web-carrito-precio-invalido de T3.6 queda superada y no hace falta mergearla, pero sigue viva en el repo. Toque tambien GASTOS.md: la linea que decia que el default del codigo era ARS 49.999 quedo mentirosa con este cambio, la actualice al precio de la casa y deje ahi la trampa del separador de miles. Deje ROADMAP-AGENTE.md sin commitear a proposito (es el marcador del piloto, no mio). Cobertura: el PDF en las dos regiones (una variable invalida cae al precio de la casa y avisa por consola), el viaje y los tres extras en las dos regiones (sin precio valido no se ofrecen), el total del carrito nunca NaN, y un test que fija que el default de AR es 85750 leido del documento y no del codigo. Falta lo que no es codigo: el catalogo de /comprar se sigue horneando en el build (eso es T3.9), asi que si PRECIO_ARS manana se rompe en Vercel la tienda vende al precio de la casa en vez de mostrar NaN, y el unico rastro es el WARN en los logs de Vercel; y desplegar esto sigue siendo subir el working tree con esta rama parada, sin cache de build.
  - cerrada: 2026-09-22T00:03:49+02:00
  - commit: dcea589
...
- [x] **T3.18** — web: el libro impreso trae el PDF incluido (hoy PDF + impreso se cobran por separado)
  - deps: —
  - tamaño: M
  - hecho-cuando: `cd web && npx vitest run && npx tsc --noEmit`
  - evidencia: cd web && npx vitest run -> 52 archivos verde | 1 skipped, 555 tests verde | 1 skipped (baseline 525); npx tsc --noEmit exit 0. Tests primero en rojo (69 de 172) y mutaciones que muerden: quitar la regla del PDF incluido -> 23 fallos; precio de la casa 98 -> 89 -> 24 fallos; archivos restaurados con el mismo SHA-256. | gate `cd web && npx vitest run && npx tsc --noEmit`: OK; exit 0
  - nota: Rama web-impreso-incluye-pdf desde main, commit 2d676bd, sin push. NO cargue variables en Vercel. Efecto visible: el impreso ahora se muestra siempre a 98 EUR / ARS 171.500 aunque las variables no esten cargadas (antes sin variable no aparecia). Dos textos nuevos de pantalla esperan aprobacion de Naza (Incluido con el impreso / Incluido con las copias) y hay una nota nueva en supabase/CONTRATO.md (extras.pdf true tambien con impreso) para confirmar con Joaquin. La copia extra sigue con el precio del impreso: es T3.19.
  - cerrada: 2026-09-22T02:04:21+02:00
  - commit: 2d676bd

- [x] **T3.19** — web: la copia extra tiene que tener su propio precio (hoy cobra el del impreso)
  - deps: T3.18
  - tamaño: S
  - hecho-cuando: `cd web && npx vitest run && npx tsc --noEmit`
  - evidencia: cd web && npx vitest run -> 53 archivos verde | 1 skipped, 583 tests verde | 1 skipped (baseline 555) y npx tsc --noEmit exit 0. Tests primero en rojo (24 fallos) y 4 de 5 mutaciones muerden, con los archivos restaurados al mismo SHA-256. | gate `cd web && npx vitest run && npx tsc --noEmit`: OK; exit 0
  - nota: Rama web-copia-extra-precio desde web-impreso-incluye-pdf (T3.18), commit 0185463, sin push. NO cargue variables en Vercel: hasta cargar PRECIO_COPIA_EXTRA_EUR y _ARS cada copia se vende al precio del impreso (98 EUR / ARS 171.500) con aviso en los logs. Precio visible que cambia: el selector de acabado del panel pasa de 98 a 40 (texto a aprobar por Naza). Sin decidir todavia: (a) el tope del descuento sigue en -20 por ciento (40 -> 32 por copia) y (b) quien compro solo el PDF y despues pide el impreso desde el panel ahora paga 40 y no 98, porque el panel no distingue a un primo de la duena que ya pago el digital: el camino en dos pasos queda 9 EUR / 15.750 ARS mas barato que el impreso del checkout. Hueco de test declarado: la linea del total del panel no se puede verificar sin DOM (los tests corren en node); el total si esta cubierto del lado del servidor. CONTRATO sin cambios.
