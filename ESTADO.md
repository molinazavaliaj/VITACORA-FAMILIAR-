...
## La vuelta del pago (3t.20, Joaquín, 21/09)

Dos pagos reales de hoy (Mariano, Nako) quedaron `pendiente` porque el webhook de Mercado Pago no
confirmó (8.6; Naza mira Vercel). Se confirmaron a mano. Para que no vuelva a pasar: el proveedor ya no
vuelve a `/comprar/gracias` sino a **`/api/pago/vuelta`** con un token firmado atado al pedido (1 hora).
La ruta consulta el pago al proveedor (`Payment.get` en MP, `checkout.sessions.retrieve` en Stripe),
confirma el pedido con `confirmarPago` (la misma del webhook, idempotente) y abre la sesión de la
compradora sin mail ni código (`generateLink` magiclink + `verifyOtp` con el `token_hash`, todo en el
servidor) → `/tablero/<narrador>`. Si algo falla, gracias como siempre. Sin migración, sin variables
