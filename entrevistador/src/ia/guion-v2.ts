...
export const GUION: readonly Fila[] = [
  ...
  { id: 'mapa-casas', etapa: 'inicio', tramo: null, inamovible: true, aplica: S, tema: 'Las casas de su vida, una tras otra. Que se sienta como un recorrido, no como un formulario. Si todavía no sabés su edad, este es el lugar para que salga sola.', pormenores: ['ciudad', 'con quién', 'hasta qué edad'] },
  ...
  { id: 'hermano', etapa: 'infancia', tramo: 'infancia', aplica: { arbol: 'hermanos' }, expandePor: 'hermanos', tema: 'Cada hermano/a {NOMBRE}, uno por uno: cómo es, cómo era de chico, la relación.', pormenores: ['qué hacían juntos', 'peleas', 'con quién se llevaba mejor', 'cómo es hoy'], siNoTuvo: variante('hijo-unico', 'Cómo era ser hijo/a único/a, sin hermanos: con quién compartía en cambio, qué extrañaba o qué disfrutaba de eso.', ['con quién compartía en cambio', 'qué extrañaba o qué disfrutaba de eso']), siNoSeSabe: puerta('hermanos-puerta', '¿Tuviste hermanos, o fuiste hijo/a único/a?') },
  ...
  // Juventud
  { id: 'a-los-quince', etapa: 'juventud', tramo: 'juventud', aplica: S, tema: 'Qué hacía a los quince fuera de la escuela, y con quién (junta la banda de esa época). Sin dar por hecho que salía.', pormenores: ['dónde paraban', 'con quién', 'qué sonaba', 'cómo se vestían', 'un sábado a la noche'] },
  { id: 'estudios', etapa: 'juventud', tramo: 'juventud', aplica: S, tema: 'Hasta dónde llegó con los estudios y cómo fue esa decisión.', pormenores: ['hasta dónde llegó', 'si lo eligió o lo eligió la vida', 'quién lo apoyó'] },
  ...
  { id: 'pareja-como-llego', etapa: 'adulto joven', tramo: 'adulto joven', aplica: { arbol: 'pareja' }, tema: 'Con quién hizo su vida y cómo llegó esa persona ({NOMBRE}). NO la boda.', pormenores: ['el día que se conocieron', 'quién dio el primer paso', 'cómo era esa persona', 'cómo lo recibieron las familias'], siNoTuvo: variante('pareja-quienes-fueron-tu-familia', 'Sin una pareja con quien hacer la vida en esos años: quiénes fueron su familia, las personas que fueron su casa.', ['quiénes fueron su familia', 'una escena']), siNoSeSabe: puerta('pareja-puerta', '¿Hubo alguien con quien hiciste tu vida?') },
  ...
  { id: 'los-hijos-creciendo', etapa: 'adultez media', tramo: 'adultez media', aplica: { arbol: 'hijos' }, tema: 'Cómo fue como madre/padre mientras crecían.', pormenores: ['qué quiso darles que no tuvo', 'qué le costó', 'una escena de la mesa o de un viaje'], siNoTuvo: variante('quienes-fueron-tu-familia', 'De quién se ocupó y quién fue su familia en esos años: las personas que fueron su casa aunque no fueran hijos.', ['de quién se ocupó', 'quién fue su familia', 'una escena']) },
...
/**
 * Pareja, pero no un complemento con "de" ("compañero de trabajo", "novia de Ariel", "mujer de mi
 * hermano"): mismo criterio que hermano/hijo/nieto. "Compañero/a" solo cuenta solo o como
 * "compañera/o de vida" (fix ronda 1, ítem B).
 */
const esPareja = (v: string) => {
  if (empieza(v, /^companer[oa]\s+de\s+vida\b/)) return true;
  return empieza(v, /^(pareja|novi[oa]|espos[oa]|marido|mujer|conyuge|companer[oa])\b(?! de\b)/);
};
...
function masNombrado(p: Perfil, nombres: string[]): string {
  ...
  for (const n of nombres) {
    const cuenta = texto.split(n).length - 1;
...
function resolver(f: Fila, arbol: Arbol, edad: number | null, eventos: ReturnType<typeof eventosDe>, p: Perfil): Resuelto {
  ...
  if ('edadMin' in c) {
    ...
    // guion §3, fila 27: entra también si la ficha dice que se jubiló o dejó de trabajar, aunque
    // no llegue a la edad; y la variante "por qué sigue" solo si lo dice explícitamente.
    if (f.id === 'dejar-el-trabajo') {
      const texto = textoDe(p);
      if (RE_JUBILO.test(texto)) return { entra: [objetivoDe(f)] };
