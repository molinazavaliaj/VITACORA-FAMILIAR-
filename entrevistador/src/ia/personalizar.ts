...
- NO PREGUNTES LO QUE ACABA DE CONTESTAR. Si una parte de la pregunta ya quedó respondida en lo que contó estos días, esa parte se reemplaza por lo que quedó abierto, no se repite. Ejemplo real: contó que con el Pelado Bausa y el Beto "no salíamos los tres juntos, creo que nunca", y la pregunta le pidió "¿cómo era salir con ellos?" — le preguntó lo que acababa de decir que no pasó. Y si acaba de negar algo ("no tengo hijos", "nunca me casé", "a mi viejo no lo vi más"), no se le pregunta por eso de ninguna forma: ni como suposición ni como condicional ("¿y si hubieras tenido...?"). Y no conviertas en el gancho algo que él nombró de paso: si menciona el alcohol o las pastillas, eso se escucha, pero vos no liderás la pregunta con eso (un narrador contó que salía "mucho vino, mucha pastilla" y la pregunta siguiente encuadró a sus amigos desde el alcohol).
- NO SUPONGAS LA VIDA DEL GUION: la pregunta se apoya en lo que él contó, nunca en lo que suele pasar. No des por sentado que la infancia fue linda (fiestas, tradiciones, "los domingos", "las travesuras que todavía lo hagan reír"), ni que hubo boda, hijos o nietos. Un narrador contestó "esto no era una película de Disney" cuando le preguntaron por los domingos familiares: si el material muestra una infancia dura o una familia desarticulada, preguntá por lo que había, quién sostenía, qué se rescataba. Y a quien no nombró hijos, nietos ni pareja no se los nombres vos.
...
// Los controles puros (cuántas preguntas, cuántas palabras, si rompe el trato) viven en
// control-texto.ts para que se puedan usar sin conectarse a la base; se re-exportan acá
// para que nada de lo que ya los importaba cambie.
export { contarPreguntas, contarPalabras, marcasDelTratoAjeno, rompeElTrato } from './control-texto.js';
import { contarPreguntas, contarPalabras, rompeElTrato } from './control-texto.js';
...
export async function personalizarPregunta(
   ...
   */
  const intento = async (prompt: string): Promise<{ ok: true; texto: string } | { ok: false; motivo: 'invalida' | 'trato' | 'fallo' }> => {
    try {
      const respuesta = await cliente().messages.create({
        model: MODELO, max_tokens: MAX_TOKENS,
    ...
    await registrarUso(db, {
      ...
      const bloque = respuesta.content.find((b) => b.type === 'text');
      const cruda = bloque && bloque.type === 'text' ? bloque.text.trim().replace(/^["'«]|["'»]$/g, '') : '';
      if (!esPersonalizacionValida(original, cruda)) return { ok: false, motivo: 'invalida' };
      // C11: la forma está bien pero le habla al narrador de otra manera.
      return rompeElTrato(cruda, trato) ? { ok: false, motivo: 'trato' } : { ok: true, texto: cruda };
    } catch (err) {
  ...
  try {
    ...
    // Decía "se manda el original" acá, antes de intentar de nuevo: cuando el
    // segundo intento salía bien, el log quedaba mintiendo. Lo que sigue lo dice.
    console.warn(`personalizar: el primer intento de la orden ${orden} de ${n.id} no sirvió (${motivoPrimero}).`);
    ...
    return {
      ...
      motivo: primero.ok
        ...
        : primero.motivo === 'trato'
          ? `la versión del modelo le hablaba en ${trato === 'vos' ? 'usted' : 'vos'} a un narrador de ${trato}`
